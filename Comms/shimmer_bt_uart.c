  ...
  if (shimmerStatus.btConnected)
  {
    ...
    switch (getCmdWaitingResponse)
    {
      ...
      case GET_STATUS_COMMAND:
      {
        ...
        *(resPacket + packet_length++) = ShimBt_assembleStatusByte();
        break;
      }
      case GET_VBATT_COMMAND:
      {
...
void ShimBt_instreamStatusRespSend(void)
{
  if (shimmerStatus.btConnected)
  {
    ...
    selfcmd[i++] = ShimBt_assembleStatusByte();
...
uint8_t ShimBt_assembleStatusByte(void)
{
  return (shimmerStatus.toggleLedRedCmd << 7) | (shimmerStatus.sdBadFile << 6)
