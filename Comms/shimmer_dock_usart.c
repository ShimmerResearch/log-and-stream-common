  ...
  { //wait for: cmd, len, data, crc -> process
    if (uartSteps == UART_STEP_WAIT4_CMD)
    {
      ...
      switch (uartAction)
      {
        ...
        default:
          uartSteps = 0;
          uartSendRspBadCmd = 1;
          crx->usb_pr_shim_dockrx_clbck_dck_cmd_rsp_task_set_count++;
    ...
    else if (uartSteps == UART_STEP_WAIT4_CRC)
    {
      ...
      if (!--uartCrc2Wait)
      {
        uartSteps = 0;
        uartArgSize = 0;
        crx->usb_pr_shim_dockrx_clbck_dck_process_cmd_task_set_count++;
...
void ShimDock_sendRsp(void)
{
  ...
  *(uartRespBuf + uart_resp_len++) = (uartRespCrc & 0xff00) >> 8;
  ...
  if (shimmerStatus.usbPluggedIn)
  {
    ...
    crx->usb_pr_shimdock_call_cdc_tx_fn_count++;
